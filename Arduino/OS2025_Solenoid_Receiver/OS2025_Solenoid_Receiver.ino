...
int debounce_press(){
  ...
  pressed_state = (pressed_state << 1) | (!digitalRead(clock)) | 0xE000;
...
int debounce_depress(){
  ...
  depressed_state = (depressed_state << 1) | (digitalRead(clock)) | 0xE000;
...
void loop() {
  if (debounce_press() and digitalRead(clock) == 1 and previous_press == false){
    receive();
    previous_press = true;
  }
  delay(0.1);
  if (debounce_depress() and digitalRead(clock) == 0 and previous_press == true){
    receive();
